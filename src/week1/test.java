...
//					}
		...
		for (Integer key : bids.keySet()){
			...
			if(reoListings.getResidences().get(in) instanceof Condo) {
				...
				for(String keyOfBids : amountOfBids.keySet()) {
					 if(keyOfBids.equals(bids.get(key))) {
						 if(amountOfBids.containsValue(0)) {
							 amountOfBids.put(bids.get(key), 1);
						 }
					 }
				}
				//if(amountOfBids.get(key))) {
					System.out.println(amountOfBids.get(key));
					amountOfBids.put(bids.get(key), k+1);
...
//				System.out.println(amountOfBids);
			...
			house.newBid(name, newBid);
			amountOfBids.put(bids.get(key), k);
			if(amountOfBids.containsKey(bids.get(key))) {
				amountOfBids.put(bids.get(key), k+1);
			System.out.println(amountOfBids);
			}
			else {
...
//}
	...
	private static void autoPopulateBids() {
		...
		Residential.newBid(autoBidders[index], bidAmount);
